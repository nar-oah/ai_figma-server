...
def get_pages(
    ...
    for canvas in file_doc.get("document", {}).get("children", []):
        ...
        out.update(
            {
                str(child.get("name", "Page")): [
                    get_node(child, refs, {}, get_ref_map(file_doc))
                ]
                for child in canvas.get("children", [])
                if isinstance(child, dict)
            }
...
def get_node(
    ...
    if node.get("type") == "INSTANCE":
        return CompNode(
            props=get_inst_props(node),
            comp=comp_refs.get(str(node.get("componentId", "")), ""),
        )
    if node.get("type") == "TEXT":
        text_ref = node.get("componentPropertyReferences", {}).get("characters")
        text = (
            "{" + prop_map[str(text_ref)] + "}"
            if text_ref and str(text_ref) in prop_map
            else str(node.get("characters", ""))
        )
        return TextNode(
            text=text, font=get_font(node, refs), color=get_color(node, refs)
        )
    return BoxNode(
...
def get_inst_props(node: dict[str, Any]) -> dict[str, str]:
    return {
        get_prop_name(str(raw), str(item.get("type", "")).lower()): str(
            item.get("value", "")
        )
...
def get_ref_map(file_doc: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        str(comp_id): str(
            comp_sets.get(str(item.get("componentSetId", "")), {}).get("name", "")
        )
...
def get_color(node: dict[str, Any], refs: TokRefs) -> str:
    ...
    return refs.color.get(
        str(styles.get("fill") or styles.get("fills") or styles.get("stroke") or ""), ""
    )
...
def get_radius(node: dict[str, Any], refs: TokRefs) -> list[str]:
    ...
    if isinstance(vals, list) and vals:
        names = [
            refs.var.get(var_refs.get(f"rectangleCornerRadii.{idx}", ""), "")
...
if __name__ == "__main__":
    ...
    def get_json(path: str) -> dict[str, object]:
        ...
        return json.loads((root / "output" / path).read_text(encoding="utf-8"))

    doc = mod_doc(get_json("api_response.json"), get_json("tokens.json"))
    add_doc_json(doc, Path("output/doc.json"))
